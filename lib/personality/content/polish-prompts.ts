...
任务：在不改变任何事实、维度分数、性格类型、判断方向的前提下，把每段文字改写得更克制、更具体、更有"被看见"的共鸣感。
...
4. 禁止以下表达：
   ...
   - 宿命论句式（"你这辈子注定""你永远都"等）
5. 每一段都必须重新遣词造句，不允许原样照抄（除非原文很短）。改写时保持原文的并列结构，不要重复或漏掉任何一个并列项。
6. 长度与原句相当（±30% 以内）。不要合并段落、不要拆分段落、不要加小标题。
