...
任务：在不改变任何事实、结论、判断方向的前提下，把每段文字改写得更具体、更有画面感、更有共鸣感，并适度丰富表达层。
...
5. 每一段都必须重新遣词造句，不允许原样照抄（除非原文极短）。保持原文的并列结构，不合并段落、不拆分段落、不加小标题。
6. 长度允许适度扩展（±30% 以内）。可以加入：
   - 互动时的微动作（"你刚想开口，他已经……""她笑了一下，没接话"）
   - 内心的犹豫和确认（"你以为这是小事，但你在意了很久"）
   - 安静的比喻（"你们的关系像是一个……"）
   但不要喧宾夺主、不要堆砌形容词。
